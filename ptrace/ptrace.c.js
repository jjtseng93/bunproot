...
function resetExecAddressSpace(pid,trampoline) {
  ...
  for (const line of mappings) {
    ...
    // Unlike execve, the emulated exec cannot teach libc that its main stack
    // moved.  Keep the mapping containing SP so bionic's cached pthread stack
    // bounds remain true for the next image as well.
    if (start<=stackPointer && end>stackPointer) continue;
...
function buildGuestStack(pid, images, guest) {
  // Keep using the kernel-created stack which contains the stopped tracee's
  // SP.  bionic caches that range in its pthread metadata; moving SP to an
  // unrelated mmap makes pthread_getattr_np() keep reporting the old range,
  // and JavaScriptCore aborts when it validates its current stack against it.
  const stackPointer=getSp(getRegisters(pid).regs);
  const stackLine=readFileSync(`/proc/${pid}/maps`,"utf8").split("\n").find((line)=>{
    const match=/^([0-9a-f]+)-([0-9a-f]+)\s+rw/.exec(line);
    return match && BigInt(`0x${match[1]}`)<=stackPointer && BigInt(`0x${match[2]}`)>stackPointer;
  });
  const stackMatch=stackLine && /^([0-9a-f]+)-([0-9a-f]+)/.exec(stackLine);
  if (!stackMatch) throw new Error(`unable to locate guest stack containing 0x${stackPointer.toString(16)}`);
  const stackBase=BigInt(`0x${stackMatch[1]}`), remoteTop=BigInt(`0x${stackMatch[2]}`);
