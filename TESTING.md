...
| `proot -S "$ROOTFS" /bin/sh -c 'd=/tmp/dac.$$; mkdir -m 555 $d && echo x > $d/f && chmod 444 $d/f && echo y > $d/f && mv $d/f $d/g && stat -c "%n %a" $d $d/g; rm -r $d'` | Fake root's `CAP_DAC_OVERRIDE`: writing into a 0555 directory and over a 0444 file, with the modes reported back as `555` and `444` afterwards. The full-size version is extracting a Fedora image with [js-udocker](https://github.com/jjtseng93/js-udocker): `JS_UDOCKER_BUNPROOT=$(realpath proot.js) bun udocker.js run --proot fedora` |
...
| `proot -S "$ROOTFS" /usr/bin/fish -c 'echo ok'` | `statx(fd, "", AT_EMPTY_PATH, ...)` on pipe descriptors stays descriptor-relative instead of trying to resolve the pipe as a directory |
